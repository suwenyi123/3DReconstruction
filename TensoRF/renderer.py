...
def evaluation(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    if PSNRs:
        ...
        mean_loss = np.mean(np.asarray(losses))  # 计算平均loss
        if compute_extra_metrics:
            ssim = np.mean(np.asarray(ssims))
            l_a = np.mean(np.asarray(l_alex))
            l_v = np.mean(np.asarray(l_vgg))
            np.savetxt(f'{savePath}/{prtx}mean.txt', np.asarray([psnr, mean_loss, ssim, l_a, l_v]))
        else:
            np.savetxt(f'{savePath}/{prtx}mean.txt', np.asarray([psnr, mean_loss]))
    else:
        mean_loss = None  # 没有计算到loss时返回None

    return PSNRs, mean_loss
